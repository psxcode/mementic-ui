...
exports.listDirNamesSync = listDirNamesSync;
exports.listDirDirnamesSync = listDirDirnamesSync;
exports.listDirFilenamesSync = listDirFilenamesSync;
...
function listDirDirnamesSync(p) {
	var entries = listDirNamesSync(p);

	if(entries == null) return entries;

	return entries.filter(function(entry) {
		return isDirSync(path.join(p, entry));
	});
}

function listDirFilenamesSync(p) {
	var entries = listDirNamesSync(p);

	if(entries == null) return entries;

	return entries.filter(function(entry) {
		return isFileSync(path.join(p, entry));
	});
}
